...
def _opencode_config_path() -> Path:
    ...
    override = (os.getenv("HARNESS_OPENCODE_CONFIG") or "").strip()
    if override:
        return Path(override)
...
_AGENT_TASKS_FILE = Path(Config.DATA_DIR) / "agent_tasks.json"
_agent_tasks_lock = threading.RLock()
_agent_scheduler_stop = threading.Event()
_agent_scheduler_thread: Optional[threading.Thread] = None


def _agent_tasks_path() -> Path:
    return Path(Config.DATA_DIR) / "agent_tasks.json"


def _normalize_task_schedule(raw: str) -> str:
    s = str(raw or "").strip().lower()
    if not s:
        return "interval:60m"
    if s == "manual":
        return "manual"
    m = re.match(r"^interval:(\d+)([mh])$", s)
    if m:
        n = max(1, int(m.group(1)))
        u = m.group(2)
        return f"interval:{n}{u}"
    d = re.match(r"^daily:(\d{2}):(\d{2})$", s)
    if d:
        hh = max(0, min(23, int(d.group(1))))
        mm = max(0, min(59, int(d.group(2))))
        return f"daily:{hh:02d}:{mm:02d}"
    return "interval:60m"


def _infer_schedule_heuristic(text: str) -> str:
    t = (text or "").strip().lower()
    if not t:
        return "interval:60m"
    if any(k in t for k in ("manual", "manualmente", "a demanda", "on demand")):
        return "manual"
    m_h = re.search(r"(cada|every)\s+(\d+)\s*(hora|horas|hour|hours|h)\b", t)
    if m_h:
        return f"interval:{max(1, int(m_h.group(2)))}h"
    m_m = re.search(r"(cada|every)\s+(\d+)\s*(min|mins|minuto|minutos|minute|minutes|m)\b", t)
    if m_m:
        return f"interval:{max(1, int(m_m.group(2)))}m"
    m_daily = re.search(r"\b(\d{1,2}):(\d{2})\b", t)
    if any(k in t for k in ("cada día", "diario", "todos los días", "every day", "daily")) and m_daily:
        hh = max(0, min(23, int(m_daily.group(1))))
        mm = max(0, min(59, int(m_daily.group(2))))
        return f"daily:{hh:02d}:{mm:02d}"
    if any(k in t for k in ("cada día", "diario", "todos los días", "every day", "daily")):
        return "daily:09:00"
    if any(k in t for k in ("semanal", "weekly", "cada semana")):
        return "daily:09:00"
    return "interval:60m"


async def _infer_task_schedule_llm(name: str, prompt: str) -> str:
    """
    Inferir schedule desde el texto de la tarea usando LLM (con fallback heurístico).
    Retorna formato normalizado.
    """
    base_text = f"{name}\n{prompt}".strip()
    heuristic = _infer_schedule_heuristic(base_text)
    try:
        client, model_name = _get_client()
        sys_prompt = (
            "Sos un parser de schedule. "
            "Leé una descripción de tarea y devolvé SOLO JSON válido con esta forma: "
            "{\"schedule\":\"interval:60m\"}. "
            "Valores permitidos: manual, interval:Xm, interval:Xh, daily:HH:MM. "
            "Sin texto adicional."
        )
        user_prompt = (
            "Descripción de tarea:\n"
            f"{base_text}\n\n"
            f"Si no es claro, usar schedule conservador. Heurística sugerida: {heuristic}.\n"
            "Respondé SOLO JSON."
        )
        resp = await _chat_create_with_fallback(
            client,
        ...
        if not m:
            return _normalize_task_schedule(txt)
        obj = json.loads(m.group(0))
        sched = str((obj or {}).get("schedule") or "").strip()
        out = _normalize_task_schedule(sched)
        return out or heuristic
    except Exception:
        return heuristic


def _agent_schedule_next_run(schedule: str, now_ts: Optional[float] = None) -> Optional[int]:
    ...
    if m:
        n = int(m.group(1))
        sec = n * 60 if m.group(2) == "m" else n * 3600
        return int(now + sec)
    d = re.match(r"^daily:(\d{2}):(\d{2})$", s)
    if d:
        hh = int(d.group(1))
        mm = int(d.group(2))
        dt_now = datetime.now()
        run_dt = dt_now.replace(hour=hh, minute=mm, second=0, microsecond=0)
...
def _load_agent_tasks() -> list[dict]:
    path = _agent_tasks_path()
    if not path.exists():
        return []
    try:
        raw = json.loads(path.read_text(encoding="utf-8"))
    ...
    for x in raw:
        if not isinstance(x, dict):
            continue
        tid = str(x.get("id") or "").strip() or uuid.uuid4().hex[:10]
        name = str(x.get("name") or "").strip()
        prompt = str(x.get("prompt") or "").strip()
        if not name or not prompt:
            continue
        schedule = _normalize_task_schedule(str(x.get("schedule") or "interval:60m"))
        status = str(x.get("status") or "active").strip().lower()
        if status not in ("active", "paused"):
            status = "paused"
        harness = str(x.get("harness") or "hermes-agent").strip().lower()
        if harness not in ("hermes-agent", "opencode", "native", "claude-code", "little-coder"):
            harness = "hermes-agent"
        next_run_at = x.get("next_run_at")
        if next_run_at is None and status == "active":
            next_run_at = _agent_schedule_next_run(schedule, now_ts)
        out.append({
            "id": tid,
            "name": name,
            "prompt": prompt,
            ...
            "harness": harness,
            "status": status,
            "created_at": int(x.get("created_at") or now_ms),
            "updated_at": int(x.get("updated_at") or now_ms),
            "last_run_at": x.get("last_run_at"),
            "last_status": str(x.get("last_status") or ""),
            "last_error": str(x.get("last_error") or ""),
            "last_output": str(x.get("last_output") or ""),
            "next_run_at": int(next_run_at) if isinstance(next_run_at, (int, float)) else None,
            "running": bool(x.get("running", False)),
        })
    return out


def _save_agent_tasks(tasks: list[dict]) -> None:
    path = _agent_tasks_path()
    _write_json_atomic(path, tasks)


def _upsert_agent_task(task: dict) -> dict:
    with _agent_tasks_lock:
        tasks = _load_agent_tasks()
        found = False
        ...
        if not found:
            tasks.append(task)
        _save_agent_tasks(tasks)
    return task


def _ensure_opencode_llamacpp_config(
    model_override: str = "",
    runtime_overrides: Optional[dict[str, Any]] = None,
) -> tuple[bool, str, str]:
    """Sincroniza OpenCode con el llama.cpp activo de UNLZ.
    Devuelve: ok, detalle, opencode_model_id
    """
    if (Config.LLM_PROVIDER or "").strip().lower() != "llamacpp":
        return True, "provider_not_llamacpp", ""

    alias = _slug_alias(str(Config.LLAMACPP_MODEL_ALIAS or "local-model"))
    runtime_cfg = _llamacpp_effective_runtime(runtime_overrides or {})
    _opencode_ctx_env = int(os.getenv("OPENCODE_CONTEXT_LIMIT", "0") or 0)
    context_limit = _opencode_ctx_env if _opencode_ctx_env > 0 else int(runtime_cfg.get("context_size") or Config.LLAMACPP_CONTEXT_SIZE or 120000)

    try:
        output_limit = int(os.getenv("OPENCODE_OUTPUT_LIMIT", "8192"))
    except Exception:
        output_limit = 8192
    try:
        provider_timeout = int(os.getenv("OPENCODE_PROVIDER_TIMEOUT_MS", "600000"))
    except Exception:
        provider_timeout = 600000
    try:
        provider_chunk_timeout = int(os.getenv("OPENCODE_PROVIDER_CHUNK_TIMEOUT_MS", "60000"))
    except Exception:
        provider_chunk_timeout = 60000
    try:
        compaction_reserved = int(os.getenv("OPENCODE_COMPACTION_RESERVED", "8192"))
    except Exception:
        compaction_reserved = 8192
    compaction_auto = str(os.getenv("OPENCODE_COMPACTION_AUTO", "true")).strip().lower() in ("1", "true", "yes", "on")
    compaction_prune = str(os.getenv("OPENCODE_COMPACTION_PRUNE", "true")).strip().lower() in ("1", "true", "yes", "on")
    disable_mcp = str(os.getenv("OPENCODE_DISABLE_MCP", "false")).strip().lower() in ("1", "true", "yes", "on")
    disable_snapshots = str(os.getenv("OPENCODE_DISABLE_SNAPSHOTS", "false")).strip().lower() in ("1", "true", "yes", "on")

    base_url = f"http://{Config.LLAMACPP_HOST}:{Config.LLAMACPP_PORT}/v1"
    model_id = f"llama-local/{alias}"
    ...
    provider = providers.get("llama-local")
    if not isinstance(provider, dict):
        provider = {}
        providers["llama-local"] = provider

    provider["name"] = "Local llama.cpp"
    ...
    options["chunkTimeout"] = provider_chunk_timeout
    ...
    current_model = models.get(alias)
    if not isinstance(current_model, dict):
        current_model = {}

    current_model.update({
        "name": str(Config.LLAMACPP_MODEL_ALIAS or alias),
        "tool_call": True,
        "limit": {
    ...
    models[alias] = current_model
    data["model"] = model_id
    data["small_model"] = model_id
    data["compaction"] = {
    ...
    if disable_mcp:
        ...
        if not isinstance(mcp_data, dict):
            mcp_data = {}
            data["mcp"] = mcp_data
        mempalace_home = str(os.getenv("OPENCODE_MEMPALACE_HOME", "")).strip()
        if mempalace_home and "mempalace" not in mcp_data:
            mcp_data["mempalace"] = {
                "type": "local",
                "command": ["mempalace-mcp"],
                "enabled": True,
                "environment": {"MEMPALACE_HOME": mempalace_home},
            }
    data["watcher"] = {
        "ignore": [
            "node_modules/**",
    ...
    try:
        sig_payload = {
            ...
            "chunk_timeout": provider_chunk_timeout,
            "compaction_auto": compaction_auto,
            "compaction_prune": compaction_prune,
            "compaction_reserved": compaction_reserved,
            "disable_mcp": disable_mcp,
            "disable_snapshots": disable_snapshots,
            "mempalace_home": str(os.getenv("OPENCODE_MEMPALACE_HOME", "")).strip(),
        }
        sig = json.dumps(sig_payload, ensure_ascii=False, sort_keys=True)
        global _OPENCODE_CONFIG_LAST_SIG
        if sig != _OPENCODE_CONFIG_LAST_SIG:
            _write_json_atomic(cfg_path, data)
            _OPENCODE_CONFIG_LAST_SIG = sig
            return True, f"opencode config sincronizado: {cfg_path}", model_id
        return True, "opencode config sin cambios", model_id
    except Exception as e:
        return False, f"No se pudo escribir opencode config: {e}", ""
...
def _has_nvidia_gpu_for_asset_pick() -> bool:
    try:
        proc = subprocess.run(
            ["nvidia-smi", "-L"],
            capture_output=True,
...
def _pick_windows_asset(assets: list[dict]) -> Optional[dict]:
    """
    Pick best llama.cpp Windows asset.
    On NVIDIA machines prefer CUDA build, then Vulkan, then CPU.
    """
    wants_cuda = _has_nvidia_gpu_for_asset_pick()
    ...
    for a in assets or []:
        ...
        is_cuda = any(x in name for x in ("cuda", "cublas", "cu11", "cu12"))
...
def _is_action_request(text: str) -> bool:
    t = (text or "").lower()
    # Evitar falsos positivos tipo "cómo se hace ..." (consulta explicativa),
    # y detectar mejor acciones imperativas reales.
    action_terms = (
        "crea", "crear", "creame", "créame", "genera", "generame", "genérame",
        "hacé", "haz", "ejecuta", "ejecutar", "corre", "run",
        "borra", "elimina", "move", "mueve", "rename", "renombra", "mkdir",
    )
    for k in action_terms:
        if _word_or_phrase_in(t, k):
            return True
    return False
...
def _should_enable_tools_auto(message: str, folder_id: str = "") -> bool:
    """Decide whether tools should be enabled in 'auto' mode.
    
    Only enable tools when the request actually needs them:
    - research/investigation queries
    - action/file-system commands
    - when a folder scope is set (RAG or sandbox context)
    
    This avoids unnecessary tool-calling overhead for simple chat.
    """
    return (
        _is_research_request(message)
        or _is_action_request(message)
        or bool(_safe_folder_id(folder_id))
    )
...
def _looks_like_multistep_build_task(message: str) -> bool:
    """
    Detecta tareas de coding/codebase que probablemente requieren múltiples pasos,
    varios archivos, documentación, refactor, revisión completa o ejecución iterativa.
    """
    t = (message or "").lower()
    ...
    coding_targets = (
        "código", "codigo", "code", "codebase", "repo", "repositorio",
        "app", "aplicación", "aplicacion", "proyecto",
        "función", "funcion", "clase", "archivo", "archivos",
        "typescript", "javascript", "python", "react", "fastapi",
        "backend", "frontend", "api", "dag", "sql", "query",
        "docker", "test", "diff", "patch", "harness", "opencode",
    )
    ...
    if any(p in t for p in multistep_phrases):
        return True

    has_target = any(_word_or_phrase_in(t, k) for k in coding_targets)
    has_intent = any(_word_or_phrase_in(t, k) for k in coding_intents)

    # Si menciona muchos archivos/proyecto/app y una acción de edición, conviene iterador/opencode.
    if has_target and has_intent:
        broad_scope_markers = (
        ...
        if any(x in t for x in broad_scope_markers):
            return True

    return False


def _word_or_phrase_in(text: str, term: str) -> bool:
    term = (term or "").strip().lower()
    if not term:
        return False

    # Frases o rutas: substring directo.
    if " " in term or "\\" in term or "/" in term:
        return term in text

    # Palabras cortas como api/sql/dag deben matchear como palabra completa.
    if len(term) <= 3:
...
def _looks_like_code_context(message: str) -> bool:
    t = message or ""
    ...
    if re.search(
        r"(?i)\b(def|class|import|from|const|let|var|function|async|await|select|with|insert|update|delete)\b",
        t,
    ):
        return True

    if re.search(
        r"(?i)\b[\w.-]+\.(py|js|ts|tsx|jsx|json|md|sql|yml|yaml|toml|env|html|css|java|go|rs|cs|cpp|c|php)\b",
        t,
    ):
        return True

    if re.search(r"(?i)([a-z]:\\|\.\\|\.\./|/home/|/users/|/mnt/)", t):
        return True

    return False


def _looks_like_coding_edit_intent(message: str) -> bool:
    t = (message or "").lower()

    edit_intents = (
        "arreglá", "arregla", "fix", "corregí", "corrige", "corregir",
        "modificá", "modifica", "editar", "editá", "edita",
        "implementá", "implementa", "implementar",
        "agregá", "agrega", "agregar",
        "creá", "crea", "crear",
        "refactor", "refactorizá", "refactoriza",
        "documentá", "documenta", "documentar",
        "comentá", "comenta", "comentar",
        "actualizá", "actualiza", "actualizar",
        "integrá", "integra", "integrar",
        "generá", "genera", "generar",
        "revisá y aplicá", "revisa y aplica",
        "hacé los cambios", "hace los cambios",
        "aplicá el patch", "aplica el patch",
    )

    return any(_word_or_phrase_in(t, k) for k in edit_intents)


def _should_use_opencode_for_message(message: str, mode: str = "normal") -> bool:
    """
    OpenCode solo para tareas reales de edición/ejecución sobre codebase.
    No debe activarse solo porque el usuario pegó código para consultar.
    """
    ...
    if _looks_like_multistep_build_task(message):
        return True

    if m == "iterate":
        return _looks_like_code_context(message) or _looks_like_coding_edit_intent(message)

    has_edit_intent = _looks_like_coding_edit_intent(message)
    ...
    coding_targets = (
        ...
        "typescript", "javascript", "python", "react", "fastapi",
        "backend", "frontend", "api", "dag", "sql", "query",
        "docker", "test", "commit", "diff", "patch", "harness", "opencode",
    )

    has_target = any(_word_or_phrase_in(t, k) for k in coding_targets)

    return has_target or _looks_like_code_context(message)
...
async def lifespan(application: FastAPI):
    ...
    _agent_scheduler_start()

    yield

    _agent_scheduler_stop_now()
...
def _build_model_chain(base_model: str, route: dict) -> list[str]:
    ...
    if provider == "llamacpp":
        # llama.cpp serves one loaded model at a time — cross-model fallback
        # chains cause long sequential timeouts. Use only the active alias.
        bm = str(base_model or Config.LLAMACPP_MODEL_ALIAS or "").strip()
...
def _normal_chat_timeout_sec() -> Optional[int]:
    try:
        v = int(str(os.getenv("NORMAL_CHAT_TIMEOUT_SEC", "45")).strip())
        if v <= 0:
            return None
...
class AgentTaskCreateRequest(BaseModel):
    name: str
    prompt: str
    schedule: str = "auto"           # auto | interval:Xm | interval:Xh | daily:HH:MM | manual
    harness: str = "hermes-agent"    # hermes-agent | opencode
    status: str = "active"           # active | paused


class AgentTaskPatchRequest(BaseModel):
...
_HARNESS_PROMPTS = {
    "native": "",
    "hermes-agent": (
        "Harness profile: hermes-agent. "
        "Act as a structured autonomous agent. "
        "First infer goal and constraints, then execute in short actionable iterations with explicit checkpoints. "
        "Prefer deterministic steps, verify outcomes quickly, and adapt if a step fails. "
        "Keep user-facing output concise while preserving execution quality."
    ),
    "claude-code": (
...
def _simple_chat_system_prompt() -> str:
    ...
    if lang == "es":
        return (
            "Sos un asistente conversacional. Respondé en español, natural y útil. "
            "Si el usuario hizo más de una pregunta, respondé cada una de forma separada y clara. "
...
async def _opencode_stream(
    ...
    bin_path = _opencode_bin()
    if not bin_path:
        yield _sse({"type": "error", "text": "Harness opencode no instalado o no encontrado en PATH. Instalá con: npm i -g opencode-ai"})
        yield _sse({"type": "done"})
        return

    if not (sandbox_root or "").strip():
        yield _sse({
            "type": "error",
            "text": "No se seleccionó ningún sandbox. Seleccioná una carpeta para usar OpenCode."
        })
        yield _sse({"type": "done"})
        return
    ...
    if not workdir_path.is_dir():
        yield _sse({
            "type": "error",
            "text": f"Sandbox inválido o inexistente: {workdir_path}"
        })
        yield _sse({"type": "done"})
        return
    ...
    # Ensure llama.cpp server is warm before calling OpenCode if using local provider.
    if (Config.LLM_PROVIDER or "").strip().lower() == "llamacpp":
        _maybe_autostart_llamacpp(force=False, reason="opencode")

        ok_switch, switch_info = await _ensure_llamacpp_model_for_override(
            model_override,
            llamacpp_overrides or {},
        )
        if not ok_switch:
            yield _sse({
                "type": "error",
                "text": f"No se pudo seleccionar el modelo para OpenCode: {switch_info}",
            })
            yield _sse({"type": "done"})
            return

        skip_warmup_if_ready = str(os.getenv("OPENCODE_SKIP_WARMUP_IF_READY", "true")).strip().lower() in ("1", "true", "yes", "on")
        must_warm = True
        # If there was no model/runtime switch and API is already healthy, skip warmup
        # to match the fast behavior of the standalone launcher.
        if skip_warmup_if_ready and str(switch_info or "") == "already_selected" and _llamacpp_api_healthy(timeout=0.8):
            must_warm = False
            yield _sse({"type": "step", "text": "llamacpp.warmup.skip", "args": {"reason": "already_selected_and_ready"}})

        if must_warm:
            yield _sse({
                "type": "step",
                "text": "llamacpp.warmup",
                "args": {"model": str(Config.LLAMACPP_MODEL_ALIAS or "")}
            })

            ok_warm, warm_info = await _ensure_llamacpp_warmup(
                str(Config.LLAMACPP_MODEL_ALIAS or "modelo")
            )
            if not ok_warm:
                yield _sse({
                    "type": "error",
                    "text": f"No se pudo preparar llama.cpp para OpenCode: {warm_info}",
                })
                yield _sse({"type": "done"})
                return

        ok_cfg, cfg_info, opencode_model_id = _ensure_opencode_llamacpp_config(
            model_override=model_override,
            runtime_overrides=llamacpp_overrides or {},
        )
        if not ok_cfg:
            yield _sse({
                "type": "error",
    ...
    minimal_prompt_mode = str(os.getenv("OPENCODE_MINIMAL_PROMPT", "true")).strip().lower() in ("1", "true", "yes", "on")
    if minimal_prompt_mode:
        # Launcher-like fast path: keep prompt tiny to reduce first-token latency.
        prompt = f"[SYSTEM]\n{_custom_sys}\n\n[USER]\n{message}" if _custom_sys else message
    else:
    ...
    # Preserve the [USER] block (current instruction) when truncating.
    _MAX_PROMPT = int(os.getenv("OPENCODE_MAX_PROMPT_CHARS", "12000"))
    if len(prompt) > _MAX_PROMPT:
        user_block = f"[USER]\n{message}"

        # Reservar siempre lugar para la instrucción actual.
        user_budget = min(len(user_block), max(2000, _MAX_PROMPT // 2))
        preserved_user = user_block[-user_budget:]

        prefix_budget = max(1000, _MAX_PROMPT - len(preserved_user) - 80)
        preserved_prefix = prompt[:prefix_budget]

        prompt = (
            preserved_prefix
            + "\n\n[...contexto anterior truncado por longitud...]\n\n"
            + preserved_user
        )
    ...
    # Si estamos usando llama.cpp, usar el provider/model que acabamos de escribir
    # en opencode.json: llama-local/<alias>.
    if opencode_model_id:
        args.extend(["--model", opencode_model_id])
    elif mo and "/" in mo:
        # Cloud/provider externo compatible con OpenCode, ej: anthropic/claude...
    ...
    if exec_mode == "confirm":
        yield _sse({
            "type": "error",
            "text": "El modo 'confirm' no es compatible con OpenCode: requiere interacción manual de permisos que el UI no puede procesar. Cambiá a 'autonomous' en Configuración → Agent Settings → Execution Mode.",
        })
        yield _sse({"type": "done"})
        return
    ...
    # Use structured stream to avoid TUI buffering/ANSI output and
    # improve first-token latency handling on Windows.
    if _opencode_supports_flag("--format"):
        args.extend(["--format", "json"])

    args.append(prompt)

    effective_model = opencode_model_id or (mo if (mo and "/" in mo) else "(opencode config)")
    yield _sse({
        "type": "step",
        "text": "opencode.args",
        "args": {
    ...
    try:
        ...
        while True:
            ...
            try:
                ...
                first_chunk_timeout = int(os.getenv("OPENCODE_FIRST_CHUNK_TIMEOUT_SEC", "45"))
                if not first_chunk_emitted and (time.time() - started_waiting_at > first_chunk_timeout):
                    ...
                    break
                silent_timeout = int(os.getenv("OPENCODE_SILENT_TIMEOUT_SEC", "300"))
            ...
            if name == "stderr":
                ...
                # Keep stderr visible for traceability without spamming the chat body.
                line = text.strip()
                if line:
                    yield _sse({"type": "step", "text": "opencode.stderr", "args": {"line": line[:500]}})
                continue

            # JSON event mode (preferred): parse line-delimited JSON payloads.
            stdout_buf += text
            ...
            for ln in lines:
                ...
                if parsed is not None:
                    ...
                    # Generic text extraction across possible event schemas.
                    txt = ""
        ...
        # Flush trailing buffered stdout content.
...
def _simple_chat_timeout_sec() -> int:
    try:
        # Default no debe ser infinito para evitar cuelgues largos percibidos en chat.
        # Permite override por env si el usuario quiere otro valor.
        raw = str(os.getenv("SIMPLE_CHAT_TIMEOUT_SEC", "60")).strip()
        v = int(raw)
        if v <= 0:
            v = 60
        return max(20, min(600, v))
    except Exception:
        return 60


def _simple_chat_history_limit() -> int:
    try:
        v = int(str(os.getenv("SIMPLE_CHAT_HISTORY_LIMIT", "2")).strip())
        return max(1, min(10, v))
    except Exception:
        return 2


def _simple_chat_token_budget(profile: str) -> int:
    p = str(profile or "quick").strip().lower()
    try:
        if p == "detailed":
            v = int(str(os.getenv("SIMPLE_CHAT_DETAILED_MAX_TOKENS", "220")).strip())
            return max(96, min(1200, v))
        v = int(str(os.getenv("SIMPLE_CHAT_QUICK_MAX_TOKENS", "96")).strip())
        return max(48, min(512, v))
    except Exception:
        return 220 if p == "detailed" else 96


def _looks_like_short_casual_message(message: str) -> bool:
    t = (message or "").strip().lower()
    if not t:
        return False
    if len(t) > 220:
        return False
    # Si parece pedido multi-step o ejecución, no aplicar fast-path.
    if _is_research_request(t) or _is_action_request(t) or _looks_like_multistep_build_task(t):
        return False
    # saludos / smalltalk / preguntas simples
    simple_markers = (
        "hola", "buenas", "que tal", "cómo estás", "como estas", "todo bien",
        "gracias", "por qué", "porque", "qué", "que", "cómo", "como",
        "cuándo", "cuando", "dónde", "donde", "quién", "quien",
    )
    if any(m in t for m in simple_markers):
        return True
    # pregunta corta sin intención de tooling
    if "?" in t and len(t) <= 120:
        return True
    return False


def _is_short_non_operational_query(message: str, folder_id: str = "") -> bool:
    """
    Preguntas cortas de chat (saludos, recetas, definiciones, etc.) que no requieren tools
    ni razonamiento largo. Se fuerza ruta rápida para minimizar latencia percibida.
    """
    t = (message or "").strip().lower()
    if not t:
        return False
    if len(t) > 180:
        return False
    if _looks_like_code_context(t):
        return False
    if _looks_like_multistep_build_task(t):
        return False
    # Si realmente necesita tools, no forzar quick.
    if _should_enable_tools_auto(t, folder_id):
        return False
    # Preguntas típicas de consulta breve.
    question_starts = (
        "que ", "qué ", "como ", "cómo ", "por que ", "por qué ",
        "cual ", "cuál ", "cuando ", "cuándo ", "donde ", "dónde ",
    )
    if any(t.startswith(x) for x in question_starts):
        return True
    if "?" in t and len(t) <= 120:
        return True
    return _looks_like_short_casual_message(t)


def _has_effective_llamacpp_overrides(overrides: Optional[dict[str, Any]]) -> bool:
    ov = _normalize_llamacpp_overrides(overrides or {})
    if not ov:
        return False
    runtime_now = _llamacpp_effective_runtime(_llamacpp_active_overrides or {})
    if "context_size" in ov and int(ov["context_size"]) != int(runtime_now.get("context_size", Config.LLAMACPP_CONTEXT_SIZE)):
        return True
    if "n_gpu_layers" in ov and int(ov["n_gpu_layers"]) != int(runtime_now.get("n_gpu_layers", Config.LLAMACPP_N_GPU_LAYERS)):
        return True
    if "flash_attn" in ov and bool(ov["flash_attn"]) != bool(runtime_now.get("flash_attn", Config.LLAMACPP_FLASH_ATTN)):
        return True
    if "cache_type_k" in ov and str(ov["cache_type_k"]).strip() != str(runtime_now.get("cache_type_k", Config.LLAMACPP_CACHE_TYPE_K or "")).strip():
        return True
    if "cache_type_v" in ov and str(ov["cache_type_v"]).strip() != str(runtime_now.get("cache_type_v", Config.LLAMACPP_CACHE_TYPE_V or "")).strip():
        return True
    if "extra_args" in ov and str(ov["extra_args"]).strip() != str(runtime_now.get("extra_args", Config.LLAMACPP_EXTRA_ARGS or "")).strip():
        return True
    return False


def _harness_latency_profile(harness_override: str = "") -> dict[str, Any]:
    """
    Per-harness latency tuning for non-opencode paths.
    These harnesses share the native runtime, but we can still reduce overhead
    with smaller context windows and lighter pre-processing.
    """
    h = _effective_harness(harness_override)
    profile: dict[str, Any] = {
        "history_limit": 12,
        "memory_enabled": True,
        "force_quick_for_short_normal": False,
    }
    if h == "claude-code":
        profile.update({
            "history_limit": 10,
            "memory_enabled": False,
            "force_quick_for_short_normal": True,
        })
    elif h == "little-coder":
        profile.update({
            "history_limit": 8,
...
def _speed_router_model_chain(base_model: str = "") -> list[str]:
    ...
    llama.cpp serves one model at a time — use only the active alias.
    Other providers can still use the env-var fallback chain.
    """
    if (Config.LLM_PROVIDER or "").strip().lower() == "llamacpp":
        bm = str(base_model or Config.LLAMACPP_MODEL_ALIAS or "").strip()
        return [bm] if bm else []
...
async def _llm_route_response_depth(
    ...
    """
    # Fast heuristic first: avoid extra LLM call for obviously short/simple prompts.
    msg_txt = str(message or "").strip()
    if _looks_like_short_casual_message(msg_txt):
        return "quick", "speed_router_heuristic_short"
    if len(msg_txt) <= 120 and not _is_action_request(msg_txt) and not _is_research_request(msg_txt) and not _looks_like_multistep_build_task(msg_txt):
        return "quick", "speed_router_heuristic_len"

    # On llama.cpp local runtime, avoid an additional router generation call
    # for medium prompts unless there are clear detailed signals.
    if (Config.LLM_PROVIDER or "").strip().lower() == "llamacpp":
        t = msg_txt.lower()
        detailed_markers = (
            "analiz", "compar", "investig", "paso a paso", "explicá", "explica",
            "profund", "detalle", "argument", "demost", "arquitect", "debug",
            "refactor", "plan", "estrateg", "implement", "código", "codigo",
        )
        if any(m in t for m in detailed_markers) or _is_action_request(msg_txt) or _is_research_request(msg_txt) or _looks_like_multistep_build_task(msg_txt):
            return "detailed", "speed_router_heuristic_detailed"
        return "quick", "speed_router_heuristic_default_llamacpp"

    client, base_model = _get_client()
    chain = _speed_router_model_chain(preferred_model or base_model)
    if not chain:
        return "quick", "speed_router_no_models"
    ...
    try:
        router_timeout = int(str(os.getenv("SPEED_ROUTER_TIMEOUT_SEC", "8")).strip())
    except Exception:
        router_timeout = 8
    router_timeout = max(4, min(12, router_timeout))
    ...
    if not choices:
        return "quick", f"speed_router_empty:{model_used}:r{retries}"
...
async def _agent_stream_normal(
    ...
    latency_profile = _harness_latency_profile(harness_override)
    history_limit = max(2, min(20, int(latency_profile.get("history_limit", 12))))
    if simple_chat:
        history_limit = _simple_chat_history_limit()
    memory_enabled = bool(latency_profile.get("memory_enabled", True))
    ...
    if simple_chat:
        ...
        try:
            ...
            profile = str(simple_chat_profile or "quick").strip().lower()
            is_detailed_simple = profile == "detailed"
            token_budget = _simple_chat_token_budget(profile)
            fast_kwargs: dict[str, Any] = {
                "messages": messages,
                # Streaming avoids long blocking waits until full completion.
                "stream": True,
                "max_tokens": token_budget,
                "temperature": 0.65 if is_detailed_simple else 0.6,
                "top_p": 0.9,
            ...
            chunk_acc: list[str] = []
            stream_any = False
            async for chunk in resp_fast:
                if cancel_event is not None and cancel_event.is_set():
                    raise asyncio.CancelledError("run cancelled by user")
                choices_fast = getattr(chunk, "choices", None) or []
                if not choices_fast:
                    continue
                delta = getattr(choices_fast[0], "delta", None)
                txt_fast = str(getattr(delta, "content", "") or "")
                if txt_fast:
                    stream_any = True
                    chunk_acc.append(txt_fast)
                    yield f"data: {json.dumps({'type': 'chunk', 'text': txt_fast})}\n\n"
            if stream_any:
                final_chunks.append("".join(chunk_acc).strip())
            if not final_chunks:
                errs = ""
                if _errors:
    ...
    if requested_tools_mode == "without_tools":
        ...
        # auto: only enable tools when the request actually needs them
        if _should_enable_tools_auto(message, folder_id):
    ...
    for iteration in range(limits["max_iterations"]):
        ...
        try:
            # Fast path: no tools → stream directly for immediate first-token.
            if not available_tools:
                _raise_if_cancelled()
                stream_kwargs: dict[str, Any] = {"messages": messages, "stream": True}
                if normal_timeout is not None:
                    stream_kwargs["timeout"] = normal_timeout
                stream_resp, model_used, retries, _errors = await _chat_create_with_fallback(
                    client,
                    model_chain,
                    cancel_event=cancel_event,
                    **stream_kwargs,
                ...
                stream_choices = getattr(stream_resp, "choices", None)
                if stream_choices:
                    stream_delta = getattr(stream_choices[0], "delta", None)
                    stream_text = getattr(stream_delta, "content", None) if stream_delta else ""
                    if stream_text:
                        final_chunks.append(stream_text)
                        yield f"data: {json.dumps({'type': 'chunk', 'text': stream_text})}\n\n"
                        emitted_final = True
                        # No tool calls in this path — emit confidence/done and return.
                        confidence = _confidence_from_context(
                            tool_calls=0,
                            had_errors=False,
                            research_request=is_research,
                            answer_text=stream_text,
                            web_result_text="",
                        )
                        yield f"data: {json.dumps({'type': 'confidence', 'score': confidence, 'tool_calls': 0})}\n\n"
                        if conversation_id:
                            try:
                                fid = _safe_folder_id(folder_id)
                                _append_memory(conversation_id, fid, "user", message)
                                _append_memory(conversation_id, fid, "assistant", stream_text)
                            except Exception:
                                pass
                        _record_router_metric(task_area, model_used, True, int((time.time() - started_at) * 1000), llm_retries_total, "normal", f"{route_reason}|stream_no_tools")
                        yield f"data: {json.dumps({'type': 'done'})}\n\n"
                        return
            ...
            if is_timeout and requested_tools_mode != "with_tools":
                try:
                    ...
                    quick_kwargs: dict[str, Any] = {
                        ...
                        "stream": False,
                        "max_tokens": _simple_chat_token_budget("detailed" if is_detailed_simple else "quick"),
                        "temperature": 0.65 if is_detailed_simple else 0.6,
                        "top_p": 0.9,
...
async def _iterate_stream(
    ...
    async def execute_stage(stage: dict, idx: int) -> tuple[bool, str, str, list[str]]:
        nonlocal model_used, llm_retries_total
    ...
    while pending:
        ...
        for stage_obj, stage_result in zip(batch, results):
            ...
            if not stage_ok:
                yield _sse({
                    "type": "chunk",
                    "text": f'No se pudo completar la etapa "{stage_name}" tras {max_retries} intentos.\n\n',
                })
...
async def _agent_stream(
    ...
    async def _stream_and_trace(inner: AsyncGenerator[str, None]) -> AsyncGenerator[str, None]:
        ...
        try:
            ...
            try:
                ...
                depth_ms = int(timing.get("depth_router_ms") or 0)
                switch_ms = int(timing.get("model_switch_ms") or 0)
                warmup_ms = int(timing.get("warmup_ms") or 0)
                router_ms = int(timing.get("router_ms") or 0)
                llm_ms = int(timing.get("llm_call_ms") or 0)
                ttft_ms = int(timing.get("ttft_ms") or 0)
                orchestration_ms = max(0, depth_ms + switch_ms + warmup_ms + router_ms)
                generation_ms = llm_ms if llm_ms > 0 else max(0, total_ms - orchestration_ms)

                summary_text = (
                    f"timing.summary total={total_ms}ms | depth={depth_ms}ms | "
                    f"model_switch={switch_ms}ms | warmup={warmup_ms}ms | router={router_ms}ms | "
                    f"generation={generation_ms}ms | ttft={ttft_ms}ms"
                )
                _devlog_emit_trace({
                    "type": "timing.summary",
                    "run_id": run_id,
                    ...
                    "args": {
                        "total_ms": total_ms,
                        "depth_router_ms": depth_ms,
                        "model_switch_ms": switch_ms,
                        "warmup_ms": warmup_ms,
                        "router_ms": router_ms,
                        "llm_call_ms": llm_ms,
    ...
    try:
        ...
        latency_profile = _harness_latency_profile(effective_harness)
        ...
        if (
            effective_harness == "opencode"
            and requested_tools_mode != "without_tools"
            and m in ("normal", "iterate")
            and _should_use_opencode_for_message(message, m)
        ):
            ...
            async for ev in _stream_and_trace(
                _opencode_stream(
                    ...
                    llamacpp_overrides=llamacpp_overrides,
                    cancel_event=cancel_event,
                )
            ):
                yield ev
        ...
        if m == "normal" and requested_tools_mode != "with_tools":
            # For non-operational chat (no tools required), always use the direct
            # chat path to avoid tool-loop latency/hangs. Keep the LLM depth router
            # only to decide response length profile (quick vs detailed).
            is_operational = _is_action_request(message) or _is_research_request(message)
            must_use_direct_chat = not is_operational
            ...
            # Si no se requieren herramientas, responder directo evita latencia
            # de task_router/tool-loop en chats puros.
            if requested_tools_mode == "without_tools" or must_use_direct_chat:
        ...
        overrides_effective = _has_effective_llamacpp_overrides(llamacpp_overrides or {})
        should_switch = not (effective_simple and not str(model_override or "").strip() and not overrides_effective)
        if should_switch:
            _phase_start("model_switch", {"model_override": str(model_override or "")})
            ok_switch, switch_info = await _ensure_llamacpp_model_for_override(
                model_override,
                (llamacpp_overrides if overrides_effective else {}),
            )
        ...
        if m == "normal" and effective_simple and requested_tools_mode != "with_tools":
            ...
            simple_profile = "detailed" if depth_route == "detailed" else "quick"
            async for ev in _stream_and_trace(_agent_stream_normal(message, history, system_prompt, model_override, harness_override, folder_id, sandbox_root, conversation_id, dry_run, simple_chat=True, simple_chat_profile=simple_profile, internet_enabled=internet_enabled, tools_mode=tools_mode, cancel_event=cancel_event)):
                yield ev
            return
...
async def _run_agent_task_once(task: dict) -> tuple[bool, str, str]:
    """
    Ejecuta una tarea programada usando el pipeline normal del agente.
    Devuelve (ok, output, err).
    """
    harness = str(task.get("harness") or "hermes-agent").strip().lower()
    ...
    errs: list[str] = []
    try:
        async for raw in _agent_stream(
            message=prompt,
            history=[],
            system_prompt="",
            model_override="",
            ...
            sandbox_root="",
            mode="normal",
            conversation_id=f"agent-task-{task.get('id')}",
            dry_run=False,
            ...
            if t == "chunk":
                ...
                errs.append(str(ev.get("text") or ""))
    except Exception as e:
        return False, "".join(chunks).strip(), _format_runtime_error(e)
    output = "".join(chunks).strip()
    if errs:
        return False, output, " | ".join([x for x in errs if x]).strip()
    if not output:
        return False, "", "empty completion"
    return True, output, ""


def _agent_scheduler_loop() -> None:
    while not _agent_scheduler_stop.is_set():
        try:
            now = int(time.time())
            with _agent_tasks_lock:
                tasks = _load_agent_tasks()
            changed = False
            for t in tasks:
                if _agent_scheduler_stop.is_set():
                    break
                if str(t.get("status")) != "active":
                    if t.get("running"):
                        ...
                        changed = True
                    continue
                if bool(t.get("running")):
                    continue
                due = t.get("next_run_at")
                if due is None:
                    due = _agent_schedule_next_run(str(t.get("schedule") or "interval:60m"), now)
                    t["next_run_at"] = due
                    changed = True
                if not isinstance(due, int) or due > now:
                    continue

                t["running"] = True
                t["updated_at"] = int(time.time() * 1000)
                changed = True
                _upsert_agent_task(t)
                _dev_log(
                    "scheduler",
                    "agent.task.run_start",
                    {
                        "task_id": t.get("id"),
                        "name": t.get("name"),
                ...
                ok, out, err = asyncio.run(_run_agent_task_once(t))
                now_ms = int(time.time() * 1000)
                next_run = _agent_schedule_next_run(str(t.get("schedule") or "interval:60m"), time.time())
                t["running"] = False
                t["last_run_at"] = now_ms
                t["last_status"] = "ok" if ok else "error"
                t["last_error"] = str(err or "")
                t["last_output"] = str(out or "")[:2400]
            ...
            _dev_log("scheduler", "agent.scheduler.error", {"error": _format_runtime_error(e)})
        _agent_scheduler_stop.wait(2.0)


def _agent_scheduler_start() -> None:
    global _agent_scheduler_thread
    if _agent_scheduler_thread and _agent_scheduler_thread.is_alive():
        return
    _agent_scheduler_stop.clear()
    th = threading.Thread(target=_agent_scheduler_loop, name="unlz-agent-scheduler", daemon=True)
    _agent_scheduler_thread = th
    th.start()


def _agent_scheduler_stop_now() -> None:
    _agent_scheduler_stop.set()
...
def _task_to_api(task: dict) -> dict:
    return {
        ...
        "schedule": str(task.get("schedule") or "interval:60m"),
        "harness": str(task.get("harness") or "hermes-agent"),
        "status": str(task.get("status") or "paused"),
        "createdAt": int(task.get("created_at") or 0),
        "updatedAt": int(task.get("updated_at") or 0),
        "lastRunAt": task.get("last_run_at"),
        "lastStatus": str(task.get("last_status") or ""),
        "lastError": str(task.get("last_error") or ""),
        "lastOutput": str(task.get("last_output") or ""),
...
async def list_agent_tasks():
    with _agent_tasks_lock:
        tasks = _load_agent_tasks()
    tasks.sort(key=lambda x: int(x.get("updated_at") or 0), reverse=True)
    return [_task_to_api(t) for t in tasks]


@app.post("/agents/tasks")
async def create_agent_task(req: AgentTaskCreateRequest):
    now_ms = int(time.time() * 1000)
    status = (req.status or "active").strip().lower()
    if status not in ("active", "paused"):
        status = "active"
    harness = (req.harness or "hermes-agent").strip().lower()
    schedule_in = str(req.schedule or "auto").strip().lower()
    if schedule_in in ("", "auto"):
        schedule = await _infer_task_schedule_llm(req.name, req.prompt)
    else:
        schedule = _normalize_task_schedule(schedule_in)
    task = {
        ...
        "last_error": "",
        "last_output": "",
        "next_run_at": _agent_schedule_next_run(schedule, time.time()) if status == "active" else None,
        "running": False,
    }
    if not task["name"] or not task["prompt"]:
        raise HTTPException(status_code=400, detail="name and prompt are required")
    _upsert_agent_task(task)
    return _task_to_api(task)


@app.patch("/agents/tasks/{task_id}")
async def patch_agent_task(task_id: str, req: AgentTaskPatchRequest):
    safe = str(task_id or "").strip()
    if not safe:
        raise HTTPException(status_code=400, detail="invalid task id")
    with _agent_tasks_lock:
        tasks = _load_agent_tasks()
        idx = next((i for i, t in enumerate(tasks) if str(t.get("id")) == safe), -1)
        if idx < 0:
            raise HTTPException(status_code=404, detail="task not found")
        t = tasks[idx]
        if req.name is not None:
            t["name"] = str(req.name).strip()
        if req.prompt is not None:
            t["prompt"] = str(req.prompt).strip()
        if req.schedule is not None:
            t["schedule"] = _normalize_task_schedule(req.schedule)
        if req.harness is not None:
            t["harness"] = str(req.harness).strip().lower()
        if req.status is not None:
            st = str(req.status).strip().lower()
            if st in ("active", "paused"):
                t["status"] = st
        if not str(t.get("name") or "").strip() or not str(t.get("prompt") or "").strip():
            raise HTTPException(status_code=400, detail="name and prompt are required")
        if str(t.get("status")) == "active":
            t["next_run_at"] = _agent_schedule_next_run(str(t.get("schedule") or "interval:60m"), time.time())
        else:
            t["next_run_at"] = None
        t["running"] = False
        t["updated_at"] = int(time.time() * 1000)
...
@app.delete("/agents/tasks/{task_id}")
async def delete_agent_task(task_id: str):
    safe = str(task_id or "").strip()
    with _agent_tasks_lock:
        tasks = _load_agent_tasks()
        next_tasks = [t for t in tasks if str(t.get("id")) != safe]
        if len(next_tasks) == len(tasks):
            raise HTTPException(status_code=404, detail="task not found")
        _save_agent_tasks(next_tasks)
    return {"success": True}


@app.post("/agents/tasks/{task_id}/run")
async def run_agent_task_now(task_id: str):
    safe = str(task_id or "").strip()
    with _agent_tasks_lock:
        tasks = _load_agent_tasks()
        idx = next((i for i, t in enumerate(tasks) if str(t.get("id")) == safe), -1)
    ...
    ok, out, err = await _run_agent_task_once(t)
    now_ms = int(time.time() * 1000)
    with _agent_tasks_lock:
        tasks = _load_agent_tasks()
        idx = next((i for i, x in enumerate(tasks) if str(x.get("id")) == safe), -1)
        if idx >= 0:
            t = tasks[idx]
            t["running"] = False
            t["last_run_at"] = now_ms
            ...
            t["last_error"] = str(err or "")
            t["last_output"] = str(out or "")[:2400]
            t["next_run_at"] = _agent_schedule_next_run(str(t.get("schedule") or "interval:60m"), time.time()) if str(t.get("status")) == "active" else None
            t["updated_at"] = now_ms
            tasks[idx] = t
            _save_agent_tasks(tasks)
    return _task_to_api(t)
...
async def _ensure_llamacpp_warmup(target_label: str = "") -> tuple[bool, str]:
    global _llamacpp_warming, _llamacpp_warming_message, _llamacpp_warm_signature, _llamacpp_last_warm_ok_ts
    if (Config.LLM_PROVIDER or "").strip().lower() != "llamacpp":
        return True, "provider_not_llamacpp"
    try:
        warm_ttl = max(0, min(3600, int(os.getenv("LLAMACPP_WARMUP_TTL_SEC", "180"))))
    except Exception:
        warm_ttl = 180
    ...
    if _llamacpp_warm_signature == sig and (time.time() - float(_llamacpp_last_warm_ok_ts or 0.0)) <= warm_ttl:
    ...
    async with _llamacpp_warmup_lock:
        ...
        try:
            ...
            if ok:
                ...
                _llamacpp_last_warm_ok_ts = time.time()
                return True, "warmed"
...
# ─────────────────────────────────────────────────────────────────────────────
# Advanced Mode — hardware info, profiles, prompt presets, benchmarks,
# launch history, health monitoring
# ─────────────────────────────────────────────────────────────────────────────

_ADVANCED_ENABLED = os.getenv("UNLZ_ADVANCED_MODE", "0").strip().lower() in ("1", "true", "yes")


def _project_root():
    """Return the project root directory (parent of agent_server.py)."""
    return Path(__file__).resolve().parent


def _adv_json_path(filename):
    """Return the path to an advanced-mode JSON file in the project root."""
    return _project_root() / filename


def _read_adv_json(filename, default=None):
    """Read a JSON file from the project root. Returns default if not found."""
    p = _adv_json_path(filename)
    if not p.exists():
        return default
    try:
        return json.loads(p.read_text(encoding="utf-8"))
...
def _write_adv_json(filename, data):
    """Write data to a JSON file in the project root."""
    p = _adv_json_path(filename)
    p.write_text(json.dumps(data, ensure_ascii=False, indent=2, default=str), encoding="utf-8")


def _detect_gpu():
    """Detect GPUs: nvidia (nvidia-smi) and apple (mps)."""
    gpus = []
    try:
            ...
            for line in result.stdout.strip().split("\n"):
                ...
                if len(parts) >= 6:
                    gpus.append({
                        ...
                        "name": parts[1],
                        "driver": parts[2],
                        "vram_total_mb": int(parts[3]) * 1024,
                        "vram_used_mb": int(parts[4]) * 1024,
                        "vram_free_mb": int(parts[5]) * 1024,
                        "type": "nvidia",
                    })
    except Exception:
        pass
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                for display in (data.get("SPDisplaysDataType") or []):
                    name = display.get("spdisplays_ngfx_model", "Apple GPU")
                    gpus.append({
                        "index": len(gpus),
                        "name": name,
                        "driver": "Apple",
            ...
            pass
    if not gpus:
        gpus.append({"index": 0, "name": "Unknown", "driver": "", "vram_total_mb": 0,
                      "vram_used_mb": 0, "vram_free_mb": 0, "type": "none"})
    return gpus
...
def _detect_models_dir(base_dir=""):
    """Scan a directory for GGUF models (mirrors OpenCode launcher model detection)."""
    if not base_dir:
        base_dir = os.path.join(os.path.expanduser("~"), "Models", "llamacpp")
    ...
    for f in sorted(models_dir.rglob("*.gguf")):
        ...
        models.append({"path": str(f), "name": f.name, "size_gb": size_gb})
    return models


# ── Default model profiles (from OpenCode launcher) ─────────────────────────

_DEFAULT_MODEL_PROFILES = {
    "Qwen3.6-35B-A3B": {
        "context_length": 131072, "max_tokens": 8192, "gpu_layers": 35,
        "temperature": 0.7, "top_p": 0.9, "repeat_penalty": 1.1,
        "kv_cache_type": "q8_0", "num_ctx": 131072, "num_batch": 512,
        "flash_attention": True, "mmap": True,
        "opencode": {"model_id": "qwen3-35b-a3b", "streaming": True},
    },
    "Llama3.1-8B": {
        "context_length": 131072, "max_tokens": 4096, "gpu_layers": 33,
        "temperature": 0.7, "top_p": 0.9, "repeat_penalty": 1.1,
        "kv_cache_type": "q8_0", "num_ctx": 8192, "num_batch": 512,
        "flash_attention": True, "mmap": True,
        "opencode": {"model_id": "llama3.1-8b", "streaming": True},
    },
    "Gemma3-4B": {
        "context_length": 131072, "max_tokens": 4096, "gpu_layers": 42,
        "temperature": 0.7, "top_p": 0.9, "repeat_penalty": 1.1,
        "kv_cache_type": "f16", "num_ctx": 8192, "num_batch": 512,
        "flash_attention": True, "mmap": True,
        "opencode": {"model_id": "gemma3-4b", "streaming": True},
    },
    "Phi-4": {
        "context_length": 16384, "max_tokens": 4096, "gpu_layers": 65,
        "temperature": 0.7, "top_p": 0.9, "repeat_penalty": 1.1,
        "kv_cache_type": "q8_0", "num_ctx": 8192, "num_batch": 512,
        "flash_attention": True, "mmap": True,
        "opencode": {"model_id": "phi-4", "streaming": True},
    },
    "DeepSeek-R1": {
        "context_length": 131072, "max_tokens": 8192, "gpu_layers": 0,
        "temperature": 0.6, "top_p": 0.9, "repeat_penalty": 1.1,
        "kv_cache_type": "q4_0", "num_ctx": 8192, "num_batch": 512,
        "flash_attention": False, "mmap": True,
        "opencode": {"model_id": "deepseek-r1", "streaming": True},
    },
    "Qwen3-30B-A3B": {
        "context_length": 262144, "max_tokens": 8192, "gpu_layers": 0,
        "temperature": 0.7, "top_p": 0.9, "repeat_penalty": 1.1,
        "kv_cache_type": "q4_0", "num_ctx": 32768, "num_batch": 512,
        "flash_attention": False, "mmap": True,
        "opencode": {"model_id": "qwen3-30b-a3b", "streaming": True},
    },
    "Qwen2.5-72B": {
        "context_length": 131072, "max_tokens": 8192, "gpu_layers": 0,
        "temperature": 0.7, "top_p": 0.9, "repeat_penalty": 1.1,
        "kv_cache_type": "q4_0", "num_ctx": 8192, "num_batch": 512,
        "flash_attention": False, "mmap": True,
        "opencode": {"model_id": "qwen2.5-72b", "streaming": True},
    },
    "Llama3-70B": {
        "context_length": 8192, "max_tokens": 4096, "gpu_layers": 0,
        "temperature": 0.7, "top_p": 0.9, "repeat_penalty": 1.1,
        "kv_cache_type": "q4_0", "num_ctx": 4096, "num_batch": 512,
        "flash_attention": False, "mmap": True,
        "opencode": {"model_id": "llama3-70b", "streaming": True},
    },
}

# ── Use-case profiles (from OpenCode launcher) ──────────────────────────────

_DEFAULT_USE_CASE_PROFILES = {
    "agentic": {
        "name": "Agentic",
        "description": "Autonomous task execution with tool use and planning",
        "settings": {
            "temperature": 0.3, "top_p": 0.9, "max_tokens": 8192,
            "repeat_penalty": 1.15, "context_length": 131072,
        },
    },
    "fast": {
        "name": "Fast",
        "description": "Quick responses for simple queries and code completion",
        "settings": {
            "temperature": 0.5, "top_p": 0.95, "max_tokens": 2048,
            "repeat_penalty": 1.05, "context_length": 8192,
        },
    },
    "balanced": {
        "name": "Balanced",
        ...
        "settings": {
            ...
            "repeat_penalty": 1.1, "context_length": 32768,
        },
    },
    "research": {
        "name": "Research",
        "description": "Deep analysis, long context, high accuracy",
        "settings": {
            "temperature": 0.2, "top_p": 0.85, "max_tokens": 16384,
            "repeat_penalty": 1.2, "context_length": 131072,
        },
    },
    "creative": {
        "name": "Creative",
        ...
        "settings": {
            ...
            "repeat_penalty": 1.05, "context_length": 16384,
        },
    },
    "coding": {
        "name": "Coding",
        ...
        "settings": {
            ...
            "repeat_penalty": 1.15, "context_length": 65536,
        },
    },
}
...
# ─────────────────────────────────────────────────────────────────────────────
# Advanced Mode — API Endpoints
# ─────────────────────────────────────────────────────────────────────────────

_ADV_PROFILE_FILE = ".unlz-profiles.json"
_ADV_PROMPT_FILE = ".unlz-prompt-presets.json"
_ADV_BENCH_FILE = ".unlz-benchmarks.json"
_ADV_HISTORY_FILE = ".unlz-launch-history.json"
...
@app.get("/advanced/hardware")
async def advanced_hardware():
    """Return hardware info: GPU, RAM, CPU, CUDA, detected models."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    gpus = _detect_gpu()
    ram = _collect_ram_stats()
    cpu_percent = _get_cpu_usage()
    models = _detect_models_dir(Config.LLAMACPP_MODEL_PATH if Config.LLM_PROVIDER == "llamacpp" else "")
    return {
        "enabled": True,
        "gpus": gpus,
...
@app.get("/advanced/profiles")
async def advanced_list_profiles():
    """List all saved profiles (model profiles + use-case profiles)."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    saved = _read_adv_json(_ADV_PROFILE_FILE, {})
    return {
        "enabled": True,
        "model_profiles": {**_DEFAULT_MODEL_PROFILES, **saved.get("model_profiles", {})},
        "use_case_profiles": {**_DEFAULT_USE_CASE_PROFILES, **saved.get("use_case_profiles", {})},
        "custom_model_profiles": saved.get("model_profiles", {}),
        "custom_use_case_profiles": saved.get("use_case_profiles", {}),
    }


@app.post("/advanced/profiles")
async def advanced_save_profile(req: ProfileApplyRequest):
    """Save a custom profile (model or use-case)."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    saved = _read_adv_json(_ADV_PROFILE_FILE, {})
    profile_type = "model_profiles" if req.model_name else "use_case_profiles"
    if profile_type not in saved:
        saved[profile_type] = {}
    profile_key = req.model_name if req.model_name else req.profile_name
    entry = {"name": req.profile_name}
    if req.model_name:
        entry["model_name"] = req.model_name
    if req.settings:
        entry["settings"] = req.settings
    saved[profile_type][profile_key] = entry
    _write_adv_json(_ADV_PROFILE_FILE, saved)
    return {"enabled": True, "saved": True, "key": profile_key}


@app.delete("/advanced/profiles/{profile_key}")
async def advanced_delete_profile(profile_key: str):
    """Delete a custom profile by key."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    saved = _read_adv_json(_ADV_PROFILE_FILE, {})
    for section in ("model_profiles", "use_case_profiles"):
        if profile_key in saved.get(section, {}):
            del saved[section][profile_key]
            _write_adv_json(_ADV_PROFILE_FILE, saved)
            return {"enabled": True, "deleted": True, "key": profile_key}
    return {"enabled": True, "deleted": False, "key": profile_key, "error": "Profile not found"}


@app.post("/advanced/profiles/apply/{profile_key}")
async def advanced_apply_profile(profile_key: str):
    """Load a profile's settings (model or use-case) to apply to current config."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    all_profiles = _read_adv_json(_ADV_PROFILE_FILE, {})
    model_profiles = {**_DEFAULT_MODEL_PROFILES, **all_profiles.get("model_profiles", {})}
    use_case_profiles = {**_DEFAULT_USE_CASE_PROFILES, **all_profiles.get("use_case_profiles", {})}
    if profile_key in model_profiles:
        return {"enabled": True, "profile_type": "model", "key": profile_key, "data": model_profiles[profile_key]}
    if profile_key in use_case_profiles:
        return {"enabled": True, "profile_type": "use_case", "key": profile_key, "data": use_case_profiles[profile_key]}
    return {"enabled": True, "error": f"Profile '{profile_key}' not found"}


@app.get("/advanced/benchmarks")
async def advanced_list_benchmarks():
    """List saved benchmark results."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    return {"enabled": True, "benchmarks": _read_adv_json(_ADV_BENCH_FILE, [])}


@app.post("/advanced/benchmarks/run")
async def advanced_run_benchmark(req: BenchmarkRunRequest):
    """Run a benchmark: measure latency, throughput, and token quality."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}

    return {
        "error": "Benchmark runner no implementado para este harness. Usá el modo nativo para benchmarks.",
        "enabled": False,
    }


@app.get("/advanced/prompt-presets")
async def advanced_list_prompt_presets():
    """List saved prompt presets (AGENTS.md / CLAUDE.md style)."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    return {"enabled": True, "presets": _read_adv_json(_ADV_PROMPT_FILE, [])}


@app.post("/advanced/prompt-presets")
async def advanced_save_prompt_preset(req: PromptPresetSaveRequest):
    """Save a prompt preset."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    presets = _read_adv_json(_ADV_PROMPT_FILE, [])
    preset_entry = {
        "name": req.name,
        "content": req.content,
        "scope": req.scope,
    ...
    _write_adv_json(_ADV_PROMPT_FILE, presets)
    return {"enabled": True, "saved": True, "name": req.name}


@app.delete("/advanced/prompt-presets/{preset_name}")
async def advanced_delete_prompt_preset(preset_name: str):
    """Delete a prompt preset by name."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    presets = _read_adv_json(_ADV_PROMPT_FILE, [])
    presets = [p for p in presets if p.get("name") != preset_name]
    _write_adv_json(_ADV_PROMPT_FILE, presets)
    return {"enabled": True, "deleted": True, "name": preset_name}


@app.post("/advanced/prompt-presets/apply/{preset_name}")
async def advanced_apply_prompt_preset(preset_name: str):
    """Load a prompt preset's content."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    presets = _read_adv_json(_ADV_PROMPT_FILE, [])
    for p in presets:
        if p.get("name") == preset_name:
            return {"enabled": True, "preset": p}
    return {"enabled": True, "error": f"Preset '{preset_name}' not found"}


@app.get("/advanced/launch-history")
async def advanced_list_launch_history():
    """List launch history entries."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    return {"enabled": True, "history": _read_adv_json(_ADV_HISTORY_FILE, [])}


@app.post("/advanced/launch-history")
async def advanced_log_launch(req: dict):
    """Log a model launch entry."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    entry = {
        ...
        "model": req.get("model", ""),
        "context_length": req.get("context_length", 0),
        "status": req.get("status", "launched"),
        "duration_s": req.get("duration_s", 0),
        "gpu": req.get("gpu", ""),
    }
    history = _read_adv_json(_ADV_HISTORY_FILE, [])
    history.insert(0, entry)
    # Keep last 200 entries
    history = history[:200]
    _write_adv_json(_ADV_HISTORY_FILE, history)
    return {"enabled": True, "logged": True}


@app.post("/actions/command")
async def run_approved_command(req: CommandActionRequest):
    """Execute a command approved by the user via the frontend UI."""
    payload = {
        "command": req.command,
        "cwd": req.cwd,
    ...
    result = _run_windows_command(payload)
...
@app.delete("/advanced/launch-history")
async def advanced_clear_launch_history():
    """Clear all launch history."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    _write_adv_json(_ADV_HISTORY_FILE, [])
    return {"enabled": True, "cleared": True}


@app.get("/advanced/health")
async def advanced_health_check():
    """Run diagnostic checks and return health status with hints."""
    if not _ADVANCED_ENABLED:
        return {"error": "Advanced mode disabled", "enabled": False}
    hints = []
    score = 100
    gpus = _detect_gpu()
    has_gpu = any(g.get("type") != "none" for g in gpus)
    if not has_gpu:
        hints.append({"level": "warning", "message": "No GPU detected — model serving will be slow or unavailable"})
        score -= 20
    cuda = _detect_cuda()
    if Config.LLM_PROVIDER == "llamacpp" and not cuda and has_gpu:
        hints.append({"level": "info", "message": "CUDA not detected — consider installing CUDA drivers for GPU acceleration"})
        score -= 10
    ram = _collect_ram_stats()
    ram_mb = ram.get("total_mb", 0)
    if ram_mb > 0 and ram_mb < 8192:
        hints.append({"level": "warning", "message": f"Low RAM ({ram_mb}MB) — models may fail to load"})
        score -= 15
    if ram_mb > 0 and ram_mb < 16384:
        hints.append({"level": "info", "message": f"RAM is {ram_mb}MB — recommend 16GB+ for larger models"})
        score -= 5
    models = _detect_models_dir(Config.LLAMACPP_MODEL_PATH if Config.LLM_PROVIDER == "llamacpp" else "")
    if not models:
        hints.append({"level": "warning", "message": "No GGUF models detected — check model path in settings"})
        score -= 20
    root = _project_root()
    env_file = root / ".env"
    if not env_file.exists():
        hints.append({"level": "error", "message": ".env file not found — create from .env.example"})
        score -= 30
    # Check agent_server.py exists
    server_file = root / "agent_server.py"
    if not server_file.exists():
        hints.append({"level": "error", "message": "agent_server.py not found"})
        score -= 30
    # Check required packages
    missing_pkgs = []
    for pkg in ("fastapi", "uvicorn", "openai", "langchain"):
        try:
            __import__(pkg)
        except ImportError:
            missing_pkgs.append(pkg)
    if missing_pkgs:
        hints.append({"level": "error", "message": f"Missing packages: {', '.join(missing_pkgs)}"})
        score -= 25
    ...
    return {
        ...
        "checks": {
            ...
            "server_file": server_file.exists(),
            "packages_ok": len(missing_pkgs) == 0,
        },
    }
...
async def chat_json_endpoint(req: ChatRequest):
    ...
    Useful for tests or clients that expect JSON instead of SSE.
    """
    chunks: list[str] = []
