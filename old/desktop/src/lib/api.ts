...
// ─── Agent tasks (scheduler backend) ────────────────────────────────────────

export type AgentTaskHarness = "hermes-agent" | "opencode" | "native" | "claude-code" | "little-coder";
...
export async function patchAgentTask(
  taskId: string,
  payload: Partial<Pick<AgentTask, "name" | "prompt" | "schedule" | "harness" | "status">>
): Promise<AgentTask> {
...
export async function runAgentTaskNow(taskId: string): Promise<AgentTask> {
  const res = await fetch(`${BASE}/agents/tasks/${encodeURIComponent(taskId)}/run`, {
    method: "POST",
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
...
export interface AdvancedHardwareInfo {
  gpu: Array<{
    index: number;
    name: string;
  ...
  cpu_cores: number;
  ram: { total_gb: number; used_gb: number; available_gb: number; usage_percent: number };
  os: string;
  python_version: string;
  detected_models: Array<{ path: string; name: string; size_gb: number }>;
  detected_models_count: number;
}

export interface AdvancedHealthInfo {
  enabled: boolean;
  status: string;
  score: number;
...
export interface AdvancedProfileData {
  ...
  use_case_profiles: Record<string, UseCaseProfile>;
...
export async function getAdvancedHardware(): Promise<AdvancedHardwareInfo> {
  const res = await fetch(`${BASE}/advanced/hardware`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

export async function getAdvancedHealth(): Promise<AdvancedHealthInfo> {
  const res = await fetch(`${BASE}/advanced/health`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

export async function getAdvancedProfiles(): Promise<AdvancedProfileData> {
  const res = await fetch(`${BASE}/advanced/profiles`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

export async function saveAdvancedProfile(payload: {
  key: string;
  profile: Record<string, unknown>;
  type: "model" | "use_case" | "custom";
...
export async function deleteAdvancedProfile(key: string): Promise<{ status: string }> {
  const res = await fetch(`${BASE}/advanced/profiles/${encodeURIComponent(key)}`, {
    method: "DELETE",
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
...
export async function applyAdvancedProfile(key: string): Promise<{ status: string }> {
  const res = await fetch(`${BASE}/advanced/profiles/apply/${encodeURIComponent(key)}`, {
    method: "POST",
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
...
export async function getAdvancedBenchmarks(): Promise<BenchmarkResult[]> {
  const res = await fetch(`${BASE}/advanced/benchmarks`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}
...
export async function runBenchmark(payload: {
  ...
  context_length: number;
  iterations?: number;
}): Promise<{ benchmark_id: string }> {
  const res = await fetch(`${BASE}/advanced/benchmarks/run`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
...
export async function deleteBenchmark(id: string): Promise<{ status: string }> {
  const res = await fetch(`${BASE}/advanced/benchmarks/${encodeURIComponent(id)}`, {
    method: "DELETE",
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
...
export async function getPromptPresets(): Promise<PromptPresetData> {
  const res = await fetch(`${BASE}/advanced/prompt-presets`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

export async function savePromptPreset(payload: {
  key: string;
  content: string;
...
export async function deletePromptPreset(key: string): Promise<{ status: string }> {
  const res = await fetch(`${BASE}/advanced/prompt-presets/${encodeURIComponent(key)}`, {
    method: "DELETE",
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
...
export async function applyPromptPreset(key: string): Promise<{ status: string; content: string }> {
  const res = await fetch(`${BASE}/advanced/prompt-presets/apply/${encodeURIComponent(key)}`, {
    method: "POST",
  });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
...
export async function getLaunchHistory(limit = 50): Promise<LaunchHistoryEntry[]> {
  const res = await fetch(`${BASE}/advanced/launch-history?limit=${limit}`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

export async function addLaunchEntry(payload: {
  profile: string;
  model: string;
  context_length: number;
  status: string;
  ...
  const res = await fetch(`${BASE}/advanced/launch-history`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
...
export async function clearLaunchHistory(): Promise<{ status: string }> {
  const res = await fetch(`${BASE}/advanced/launch-history`, { method: "DELETE" });
  if (!res.ok) throw new Error(await res.text());
  return res.json();
}
