...
function BehaviorEditor({
  ...
  const [harness, setHarness] = useState(behavior.harness ?? "opencode");
  ...
  return (
    <div className="flex flex-col gap-4 p-5 bg-surface border border-border rounded-xl">
      ...
      <div className="flex items-center gap-3">
        <div className="relative group">
          ...
          <div className="absolute top-12 left-0 z-10 hidden group-focus-within:flex flex-wrap gap-1 bg-raised border border-border rounded-lg p-2 shadow-xl w-48">
            {EMOJI_PRESETS.map((e) => (
              <button key={e} onClick={() => setIcon(e)} className="w-7 h-7 text-base rounded hover:bg-base transition-colors">
                {e}
              </button>
            ))}
          </div>
      ...
      <div className="flex items-center gap-1.5">
        ...
        {EMOJI_PRESETS.map((e) => (
          <button
            key={e}
            onClick={() => setIcon(e)}
            className={`w-7 h-7 text-base rounded transition-colors ${icon === e ? "bg-accent/20 ring-1 ring-accent/50" : "hover:bg-raised"}`}
          >
            {e}
          </button>
        ))}
      ...
      {/* System prompt */}
      <div>
        <label className="block text-xs font-medium text-muted mb-1.5">System prompt (Markdown soportado)</label>
        <textarea
          value={content}
      ...
      <div>
        <label className="block text-xs font-medium text-muted mb-1.5">Modelo (opcional)</label>
        <input
          value={model}
          onChange={(e) => setModel(e.target.value)}
          placeholder="ej: anthropic/claude-sonnet-4-5"
        ...
        />
        <p className="text-[10px] text-muted mt-1">Formato provider/model-id para opencode.</p>
      </div>

      <div>
        <label className="block text-xs font-medium text-muted mb-1.5">Harness</label>
        <select
          value={harness}
      ...
      <div className="flex items-center justify-end gap-2">
        <button onClick={onCancel} className="btn-ghost flex items-center gap-1.5 px-3 py-1.5 text-sm">
          <X size={14} /> Cancelar
        </button>
        <button
          onClick={() => {
            if (!name.trim() || !content.trim()) return;
            onSave(name.trim(), content, icon, model.trim(), harness.trim(), defaultInternetEnabled, defaultToolsMode);
          }}
...
function BehaviorCard({
  ...
  return (
    <div className="group flex flex-col gap-3 p-4 bg-surface border border-border rounded-xl hover:border-border-strong transition-colors hover:border-[#3a3a60]">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            {isDefault && (
              <span className="text-[10px] text-muted bg-raised px-1.5 py-0.5 rounded font-medium">predeterminado</span>
            )}
          </div>
        </div>
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button onClick={onEdit} className="p-1.5 rounded-md text-muted hover:text-primary hover:bg-raised transition-colors" title="Editar">
            <Pencil size={13} />
          </button>
          {!isDefault && (
            <button onClick={onDelete} className="p-1.5 rounded-md text-muted hover:text-red-400 hover:bg-raised transition-colors" title="Eliminar">
              <Trash2 size={13} />
            </button>
          )}
        </div>
      ...
      {behavior.model?.trim() && (
        <p className="text-[10px] text-muted font-mono">Modelo: {behavior.model}</p>
      )}
      {behavior.harness?.trim() && (
        <p className="text-[10px] text-muted font-mono">Harness: {behavior.harness}</p>
      )}
      <p className="text-[10px] text-muted font-mono">
        Internet: {behavior.defaultInternetEnabled === false ? "off" : "on"} · Tools: {behavior.defaultToolsMode || "auto"}
      </p>

      <button onClick={onUse} className="btn-ghost text-xs px-3 py-1.5 self-start flex items-center gap-1.5">
        <Brain size={12} /> Usar en nueva conversación
      </button>
    </div>
  );
}
...
type EditorState = { mode: "new" } | { mode: "edit"; behavior: Behavior } | null;

export default function BehaviorsView() {
  const { behaviors, createBehavior, updateBehavior, deleteBehavior, createConversation } = useStore();
  const [editor, setEditor] = useState<EditorState>(null);

  function handleSaveNew(
    name: string, content: string, icon: string, model: string, harness: string,
    defaultInternetEnabled: boolean, defaultToolsMode: "auto" | "with_tools" | "without_tools",
  ) {
    createBehavior(name, content, icon, model, harness, defaultInternetEnabled, defaultToolsMode);
    setEditor(null);
  }

  function handleSaveEdit(
    id: string, name: string, content: string, icon: string, model: string, harness: string,
    defaultInternetEnabled: boolean, defaultToolsMode: "auto" | "with_tools" | "without_tools",
  ) {
    updateBehavior(id, { name, content, icon, model, harness, defaultInternetEnabled, defaultToolsMode });
    setEditor(null);
  }
    ...
    <div className="flex flex-col h-full overflow-hidden">
      <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">Perfiles de system prompt — definen cómo responde el agente</p>
        </div>
        {!editor && (
          <button onClick={() => setEditor({ mode: "new" })} className="btn-primary flex items-center gap-1.5 px-3 py-1.5 text-sm">
            <Plus size={14} /> Nuevo
          </button>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-6 py-5 custom-scroll">
        {editor && (
          <div className="mb-6">
            ...
            {editor.mode === "new" ? (
              <BehaviorEditor behavior={{}} onSave={handleSaveNew} onCancel={() => setEditor(null)} />
            ) : (
              <BehaviorEditor
                behavior={editor.behavior}
                onSave={(name, content, icon, model, harness, defaultInternetEnabled, defaultToolsMode) =>
                  handleSaveEdit(editor.behavior.id, name, content, icon, model, harness, defaultInternetEnabled, defaultToolsMode)
                }
        ...
        {behaviors.length === 0 && !editor && (
          <div className="flex flex-col items-center justify-center py-20 gap-3">
            ...
            <button onClick={() => setEditor({ mode: "new" })} className="btn-primary text-sm px-4 py-2 mt-1">
              Crear el primero
            </button>
          </div>
        )}
